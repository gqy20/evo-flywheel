...
| M2 | v0.1.1 | ✅ | 数据采集层 (RSS、bioRxiv API、去重、编排器、调度器) |
| M3 | v0.1.2 | ✅ | LLM 分析层 (论文分析、批量处理、提示词) |
| M4 | v0.1.3 | ✅ | 搜索层 (嵌入、存储、语义搜索、混合搜索) |
| M5 | v0.1.4 | ✅ | Web 界面 (Streamlit: 首页、列表、搜索、报告) |
| M6 | v0.1.5 | ✅ | 测试优化与部署 |
| M7 | v0.1.6 | ✅ | FastAPI 后端 (REST API) |
| M8 | v0.1.7 | ✅ | Web UI 后端集成 (APIClient、论文详情、反馈功能) |
| M9 | v0.1.8 | ✅ | 飞轮控制 (自动调度、深度报告、多报告支持) |
